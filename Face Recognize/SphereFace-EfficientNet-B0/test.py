...
MODEL_NAME = 'sphereface_efficientnet_b0.pth'
CLASS_MAPPING_NAME = 'class_mapping.json'

# Hyperparameters
IMG_SIZE = 224
# Embedding size is determined by the backbone, 1280 for EfficientNet-B0

def test():
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"Using device: {device}")

    # --- Load Class Mapping and the Trained Backbone ---
    ...
    # Load the backbone model structure (without classifier)
    ...
    true_labels, pred_labels, inference_times = [], [], []
    ...
    with torch.no_grad():
        for image, label in test_loader:
            ...
            similarities = {
                class_idx: F.cosine_similarity(query_embedding, gallery_emb.unsqueeze(0)).item()
    ...
    plt.title('Confusion Matrix (SphereFace + EfficientNet-B0)')
    plt.show()
