...
import matplotlib.pyplot as plt # เพิ่ม import นี้
...
MODEL_NAME = 'facenet_efficientnet_b0.pth'
CLASS_MAPPING_NAME = 'class_mapping.json'
PLOT_SAVE_PATH = 'training_history_facenet_finetune.png' # เปลี่ยนชื่อไฟล์ plot
...
class TripletDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # Make sure positive is not the same image as anchor, but can be if only 1 image per class.
        # This implementation ensures different *indices* in the dataset.
...
def plot_training_history(history):
    ...
    plt.title('Model Triplet Loss (FaceNet Fine-tuning)')
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
    plt.legend()
    plt.grid(True)
...
def train():
    ...
    train_transform = transforms.Compose([
        ...
        transforms.RandomAffine(degrees=10, translate=(0.05, 0.05), scale=(0.95, 1.05), shear=5), # เพิ่ม data augmentation เหมือน ArcFace
    ...
    model = FaceNetModel(embedding_size=EMBEDDING_SIZE, pretrained=True).to(device) # pretrained=True for Fine-tuning
    criterion = nn.TripletMarginLoss(margin=TRIPLET_MARGIN, p=2) 
    optimizer = optim.Adam(model.parameters(), lr=LEARNING_RATE, weight_decay=1e-5) # เพิ่ม weight_decay เพื่อช่วยป้องกัน Overfitting
