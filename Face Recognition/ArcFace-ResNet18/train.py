...
PLOT_SAVE_PATH = 'training_history_resnet18_arcface.png'
...
def plot_training_history(history, best_epoch):
    """ฟังก์ชันวาดกราฟที่เพิ่มการ Plot จุด"""
    ...
    plt.title(f'Model Accuracy ({BACKBONE_NAME} + ArcFace)')
    ...
    plt.axvline(x=best_epoch, color='r', linestyle='--', label=f'Best Epoch: {best_epoch+1}')
    plt.title(f'Model Loss ({BACKBONE_NAME} + ArcFace)')
...
def train():
    ...
    print(f"Starting training {BACKBONE_NAME} with ArcFace Loss...")
