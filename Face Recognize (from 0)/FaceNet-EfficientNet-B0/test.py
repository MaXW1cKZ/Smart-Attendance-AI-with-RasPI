...
# Hyperparameters (match with training)
IMG_SIZE = 224
EMBEDDING_SIZE = 512
...
def test():
    ...
    model = FaceNetModel(embedding_size=EMBEDDING_SIZE, pretrained=False)
    ...
    # Important: Use a large batch size for gallery creation to speed it up
    ...
    with torch.no_grad():
        for images, labels in train_loader:
            ...
            embeddings = F.normalize(model.get_embedding(images))
    ...
    # Average embeddings for each class to create a single representative embedding
    ...
    with torch.no_grad():
        for image, label in test_loader:
            ...
            query_embedding = F.normalize(model.get_embedding(image))
            end_time = time.time()
            inference_times.append(end_time - start_time)

            # Compare with gallery using Cosine Similarity
            similarities = {}
    ...
    plt.title('Confusion Matrix (FaceNet train from 0 + EfficientNet-B0)')
    plt.show()
